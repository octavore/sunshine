...
@Suite struct SkipAndRemindStoreTests {
    ...
    private func makeStore() -> (SkipAndRemindStore, UserDefaults, String) {
        ...
        return (SkipAndRemindStore(bundleIdentifier: "test.app", defaults: defaults), defaults, suiteName)
    }

    private func update(tag: String) -> Update {
        Update(
            release: GitHubRelease(tagName: tag),
            asset: GitHubAsset(name: "App-\(tag).zip", browserDownloadURL: URL(string: "https://example.com/\(tag).zip")!, size: 1)
        )
    }
