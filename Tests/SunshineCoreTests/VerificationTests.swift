...
@Suite struct VerificationTests {
    ...
    @Test func teamIDMismatchFails() {
        let verifier = UpdateVerifier(requireNotarization: true, checker: MockChecker(candidateTeamID: "OTHER"))
        #expect(throws: SunshineError.self) { try verifier.verify(appAt: url) }
    }

    @Test func unsignedCandidateFails() {
        let verifier = UpdateVerifier(requireNotarization: true, checker: MockChecker(candidateTeamID: nil))
        #expect(throws: SunshineError.self) { try verifier.verify(appAt: url) }
    }

    @Test func invalidSignatureFails() {
        let verifier = UpdateVerifier(requireNotarization: true, checker: MockChecker(signatureError: DummyError()))
        #expect(throws: SunshineError.self) { try verifier.verify(appAt: url) }
    }
    ...
    @Test func notarizationSkippedWhenNotRequired() throws {
        let verifier = UpdateVerifier(requireNotarization: false, checker: MockChecker(notarized: false))
        let report = try verifier.verify(appAt: url)
        #expect(report.teamIdentifierMatches)
    }

    @Test func unsignedRunningAppFails() {
        let verifier = UpdateVerifier(requireNotarization: true, checker: MockChecker(runningTeamID: nil))
        #expect(throws: SunshineError.self) { try verifier.verify(appAt: url) }
    }
}
