...
enum ExampleData {
    ...
    static func release(tag: String, prerelease: Bool = false, daysAgo: Int = 1, includeNotes: Bool = true) -> GitHubRelease {
        GitHubRelease(
            tagName: tag,
            name: tag,
    ...
    static func releases(for config: ExampleReleaseConfig) -> [GitHubRelease] {
        [release(tag: config.latestVersion, prerelease: config.isPrerelease, daysAgo: 1, includeNotes: config.includeReleaseNotes)]
            + releases.dropFirst()
    }
