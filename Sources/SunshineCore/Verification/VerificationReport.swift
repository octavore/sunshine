public enum VerificationFailure: Error, Sendable, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .teamIdentifierMismatch(let expected, let found):
            return "Downloaded update's Team ID (\(found ?? "none")) does not match the running app's Team ID (\(expected ?? "none"))."
        case .teamIdentifierMissing:
        ...
        case .runningAppUnsigned:
            return "The running app itself has no Team ID to compare against; refusing to install any update."
        }
    }
}
