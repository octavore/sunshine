...
enum ArchiveExtractor {
    ...
    static func extractApp(fromArchiveAt archiveURL: URL, into destinationDirectory: URL) throws -> URL {
    ...
    private static func extractDMG(_ archiveURL: URL, into destinationDirectory: URL) throws -> URL {
        ...
        try run("/usr/bin/hdiutil", ["attach", archiveURL.path, "-mountpoint", mountPoint.path, "-nobrowse", "-readonly", "-quiet"])
